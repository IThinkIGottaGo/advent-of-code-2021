 ...
 *
 * 信标和扫描器一动不动的漂浮在水中；它们设计出来就是为了能够长时间的保持相同位置的。每个扫描器都可以以自身为中心，在一个大的立方体范围中，侦测到所有属于这个范围中的信标；
 * 在三轴 (`x`, `y` 和 `z`) 中的每个轴都确定了信标相对于扫描器的精确位置，且扫描器能侦测到的信标最多 1000 个单位远。但是扫描器不能侦测到其他的扫描器。
 * 潜艇已经自动统计了由扫描器侦测到的各个信标的相对位置（这便是你的谜题输入）。
 *
