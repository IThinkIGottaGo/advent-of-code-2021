 ...
 *
 * 现在你的目标是计算**每次滑动窗口中测量值的总和，都比上一次滑动窗口总和更大的次数**。先比较 A 和 B，然后比较 B 和 C，再是 C 和 D，以此类推。
 ...
 * ```
 *
 * 在这个例子中，总共有 **5** 次是这次的总和大于上一次的总和。
 *
 * 第二个问题：考虑由三次测量总和组成的滑动窗口。**这次总和比上一次总和更大的情况共有多少次？**
 */
fun main() {
    // 第一个问题
    ...
    check(part1(input) == 1400)
    check(part2(input) == 1429)
}